...
**零改动**。`_source` 中存储的原始 JSON 不受映射类型影响，`clean_document` 中的 `add_dimension_display_name` 逻辑不变。
...
| 维度 key | ID 字段名 | ES 查询路径 | 值类型 | 示例值 |
|---------|----------|------------|--------|--------|
| `set` | `set_id` | `impact_scope.set.instance_list.set_id` | string | `"5179871"` |
| `host` | `bk_host_id` | `impact_scope.host.instance_list.bk_host_id` | string* | `"9185731"` |
| `service_instances` | `bk_service_instance_id` | `impact_scope.service_instances.instance_list.bk_service_instance_id` | string* | `"12345"` |
| `cluster` | `bcs_cluster_id` | `impact_scope.cluster.instance_list.bcs_cluster_id` | string | `"BCS-K8S-00001"` |
| `node` | `node` | `impact_scope.node.instance_list.node` | string | `"node-01"` |
| `service` | `service` | `impact_scope.service.instance_list.service` | string | `"svc-01"` |
| `pod` | `pod` | `impact_scope.pod.instance_list.pod` | string | `"pod-01"` |
| `apm_app` | `app_name` | `impact_scope.apm_app.instance_list.app_name` | string | `"my-app"` |
| `apm_service` | `service_name` | `impact_scope.apm_service.instance_list.service_name` | string | `"my-svc"` |
...
| conditions key | ES 查询字段 | 查询类型 | 说明 |
|---------------|------------|---------|------|
| `impact_dimensions` | `impact_scope.{dim}` | `exists` | 维度级：判断是否包含某维度 |
| `impact_scope.set.set_id` | `impact_scope.set.instance_list.set_id` | `terms` | 按 Set ID 过滤 |
| `impact_scope.host.bk_host_id` | `impact_scope.host.instance_list.bk_host_id` | `terms` | 按主机 ID 过滤 |
| `impact_scope.service_instances.bk_service_instance_id` | `impact_scope.service_instances.instance_list.bk_service_instance_id` | `terms` | 按服务实例 ID 过滤 |
| `impact_scope.cluster.bcs_cluster_id` | `impact_scope.cluster.instance_list.bcs_cluster_id` | `terms` | 按 BCS 集群 ID 过滤 |
| `impact_scope.node.node` | `impact_scope.node.instance_list.node` | `terms` | 按 K8S 节点名过滤 |
| `impact_scope.service.service` | `impact_scope.service.instance_list.service` | `terms` | 按 K8S Service 名过滤 |
| `impact_scope.pod.pod` | `impact_scope.pod.instance_list.pod` | `terms` | 按 K8S Pod 名过滤 |
| `impact_scope.apm_app.app_name` | `impact_scope.apm_app.instance_list.app_name` | `terms` | 按 APM 应用名过滤 |
| `impact_scope.apm_service.service_name` | `impact_scope.apm_service.instance_list.service_name` | `terms` | 按 APM 服务名过滤 |
...
```json
{
    "impact_scope": {
        ...
        "service_instances": {
            "count": 2,
            "instance_list": [
                {"bk_service_instance_id": 12345, "display_name": "mysql_exporter(21.249.64.16)"},
                {"bk_service_instance_id": 67890, "display_name": "node_exporter(21.186.179.6)"}
            ],
            "link_tpl": null
        },
        "cluster": {
            "count": 1,
            "instance_list": [
                {"bcs_cluster_id": "BCS-K8S-00001", "display_name": "生产集群(BCS-K8S-00001)"}
            ],
        ...
        "node": {
            "count": 2,
            "instance_list": [
                {"bcs_cluster_id": "BCS-K8S-00001", "node": "node-01", "display_name": "BCS-K8S-00001/node-01"},
                {"bcs_cluster_id": "BCS-K8S-00001", "node": "node-02", "display_name": "BCS-K8S-00001/node-02"}
            ],
        ...
        "apm_service": {
            "count": 2,
            "instance_list": [
                {"app_name": "my-app", "service_name": "order-svc", "bk_biz_id": 2, "display_name": "my-app/order-svc"},
                {"app_name": "my-app", "service_name": "pay-svc", "bk_biz_id": 2, "display_name": "my-app/pay-svc"}
            ],
