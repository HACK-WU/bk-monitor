# Join Node Configuration (关联节点配置)

## 节点类型
- **NodeType**: `join`
- **分类**: DATA_PROCESSING (数据处理类)
- **功能**: 关联多个事件流，基于关联键进行合并

## 配置 Schema

### JoinNodeConfigSerializer
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `node_type` | string | 是 | "join" | 节点类型标识 |
...
1. **场景一**：关联多个事件流，基于关联键进行合并的基础应用
2. **场景二**：与其他节点配合使用
3. **场景三**：复杂业务逻辑处理

## 注意事项

1. 确保 `node_type` 字段值为 `join`
2. 配置字段需符合 Schema 定义
3. 建议启用错误处理和重试机制
