# Trend Node Configuration (趋势检测节点配置)

## 节点类型
- **NodeType**: `trend`
- **分类**: DETECTION (检测类)
- **功能**: 检测指标趋势变化，如突增、突降、持续上涨等

## 配置 Schema

### TrendNodeConfigSerializer
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `node_type` | string | 是 | "trend" | 节点类型标识 |
...
1. **场景一**：检测指标趋势变化，如突增、突降、持续上涨等的基础应用
2. **场景二**：与其他节点配合使用
3. **场景三**：复杂业务逻辑处理

## 注意事项

1. 确保 `node_type` 字段值为 `trend`
2. 配置字段需符合 Schema 定义
3. 建议启用错误处理和重试机制
