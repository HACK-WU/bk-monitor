# Delay Node Configuration (延迟节点配置)

## 节点类型
- **NodeType**: `delay`
- **分类**: FLOW_CONTROL (流控类)
- **功能**: 延迟事件处理，等待一段时间后再流转

## 配置 Schema

### DelayNodeConfigSerializer
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `node_type` | string | 是 | "delay" | 节点类型标识 |
...
1. **场景一**：延迟事件处理，等待一段时间后再流转的基础应用
2. **场景二**：与其他节点配合使用
3. **场景三**：复杂业务逻辑处理

## 注意事项

1. 确保 `node_type` 字段值为 `delay`
2. 配置字段需符合 Schema 定义
3. 建议启用错误处理和重试机制
