# Callback Node Configuration (回调节点配置)
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `node_type` | string | 是 | "callback" | 节点类型标识 |
...
```json
{
  "name": "callback_advanced",
  "description": "回调节点高级配置示例",
  "enabled": true,
  "node_type": "callback",
  "skip_condition": {
    "logic": "and",
...
1. **场景一**：回调外部系统的API的基础应用
2. **场景二**：与其他节点配合使用
3. **场景三**：复杂业务逻辑处理

## 注意事项

1. 确保 `node_type` 字段值为 `callback`
2. 配置字段需符合 Schema 定义
3. 建议启用错误处理和重试机制
