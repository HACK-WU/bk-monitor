...
class AccessHandler(base.BaseHandler):
    ...
    def handle_event_v2(self):
        ...
        └─────────────────────────────────────────────────────────────────┘
                                  ...
                                  ▼ (GSE Agent 推送)
                            ┌───────────┐
                            │   Kafka   │  ← 生产者是 GSE Agent
                            │   Topic   │
                            └───────────┘
                                  │
                                  ▼ (EventPoller 消费)
        ┌─────────────────────────────────────────────────────────────────┐
        │  bkmonitor - handle_event_v2() → EventPoller.start()            │
        │  从 Kafka 拉取数据 → 推送到 Redis → 触发 Celery 任务处理          │
        └─────────────────────────────────────────────────────────────────┘

        为什么 GSE 选择推送到 Kafka 而不是 Redis:

        1. 数据持久化和可靠性
           - Kafka 提供持久化存储，数据写入磁盘并支持副本机制，保证数据不丢失
           - Redis 主要是内存存储，宕机或重启可能导致数据丢失
           - Kafka 支持偏移量管理和回溯消费，消费者宕机后可以从上次位置继续

        2. 数据量和吞吐量
           - GSE Agent 产生海量数据(成千上万台主机同时上报，秒级高频采集)
           - Kafka 支持批量处理(max_partition_fetch_bytes=5MB，msg_push_batch_size=1000)
           - Redis 单线程处理，高并发下性能受限，内存占用会急剧增长

        3. 消息队列语义
           - Kafka 原生支持消费者组(Consumer Group)，实现多实例负载均衡
           - Kafka 分区机制天然支持并行处理，自动重平衡(Rebalance)
           - Kafka 精确的偏移量管理，而 Redis 需要手动实现这些能力

        4. 解耦和异步处理
           - GSE 和 bkmonitor 完全解耦，互不影响，任一方维护不影响另一方
           - GSE 推送数据后立即返回，不需要等待处理
           - bkmonitor 可以按自己的节奏消费，削峰填谷

        5. 架构分层
           - Kafka: 大规模数据接入层 (GSE事件、告警事件推送)
           - Redis: 轻量级缓冲和信号通知层 (Kafka到Celery任务的桥梁)
           - 两者配合使用，各司其职

        订阅的数据源 (EventPoller.refresh()):
           - GSE_BASE_ALARM_DATAID (1000): GSE基础告警
           - GSE_CUSTOM_EVENT_DATAID (1100000): GSE自定义事件
           - GSE_PROCESS_REPORT_DATAID (1100008): GSE进程上报
