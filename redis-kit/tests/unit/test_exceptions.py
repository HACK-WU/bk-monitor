...
class TestExceptionHierarchy:
    ...
    def test_configuration_errors(self):
        ...
        exc2 = InvalidConfigError("invalid")
        exc3 = MissingConfigError("missing")
    ...
    def test_connection_errors(self):
        ...
        exc2 = ConnectionTimeoutError("timeout")
        exc3 = ConnectionRefusedError("refused")
        exc4 = AuthenticationError("auth failed")
        
        assert isinstance(exc1, RedisKitError)
        assert isinstance(exc2, ConnectionError)
        assert isinstance(exc3, ConnectionError)
        assert isinstance(exc4, ConnectionError)
    ...
    def test_key_errors(self):
        """测试键相关异常"""
        exc1 = KeyError("key error")
        exc2 = KeyTemplateError("invalid template")
        exc3 = KeyFormatError("format error")
        
        assert isinstance(exc1, RedisKitError)
        assert isinstance(exc2, KeyError)
        assert isinstance(exc3, KeyError)
