...
class HashKey(RedisDataKey):
    ...
    def __init__(self, key_tpl: str = None, ttl: int = None, backend: str = None, field_tpl: str = None, **extra_config):
    ...
    def hset(self, value: str, field_kwargs: Dict[str, Any] = None, **key_kwargs) -> int:
        """设置 Hash 字段值"""
        key = self.get_key(**key_kwargs)
        field = self.get_field(**(field_kwargs or {}))
    ...
    def hincrby(self, amount: int = 1, field_kwargs: Dict[str, Any] = None, **key_kwargs) -> int:
        """递增 Hash 字段值"""
        key = self.get_key(**key_kwargs)
        field = self.get_field(**(field_kwargs or {}))
...
class SortedSetKey(RedisDataKey):
    ...
    def zadd(self, mapping: Dict[str, float], nx: bool = False, xx: bool = False, **key_kwargs) -> int:
        """添加成员到有序集合"""
        key = self.get_key(**key_kwargs)
    ...
    def zrange(self, start: int = 0, end: int = -1, withscores: bool = False, **key_kwargs) -> List:
        """获取范围内的成员（按分数从小到大）"""
        key = self.get_key(**key_kwargs)
        return self.client.zrange(key, start, end, withscores=withscores)

    def zrevrange(self, start: int = 0, end: int = -1, withscores: bool = False, **key_kwargs) -> List:
        """获取范围内的成员（按分数从大到小）"""
        key = self.get_key(**key_kwargs)
        return self.client.zrevrange(key, start, end, withscores=withscores)

    def zrangebyscore(
        self, min_score: float, max_score: float, start: int = None, num: int = None, withscores: bool = False, **key_kwargs
    ) -> List:
        """按分数范围获取成员"""
        key = self.get_key(**key_kwargs)
        return self.client.zrangebyscore(key, min_score, max_score, start=start, num=num, withscores=withscores)
