...
from redis_kit.exceptions import TaskAlreadyExistsError, TaskNotFoundError
...
class DelayQueue:
    ...
    def pop_ready(self, batch_size: int = 100) -> List[Tuple[str, Any]]:
        ...
        # 获取到期的任务 ID
        task_ids = self.client.zrangebyscore(self.queue_name, 0, now, start=0, num=batch_size)
        ...
        # 只处理成功移除的任务
        removed_ids = [task_id for task_id, removed in zip(task_ids, results) if removed]
    ...
    def reschedule(self, task_id: str, delay_seconds: float = 0, execute_at: float = None) -> bool:
        """
        重新调度任务
...
class DelayQueueWorker:
    ...
    def _default_error_handler(self, task_id: str, task_data: Any, error: Exception) -> None:
        """默认错误处理"""
