...
class RoutingStrategyProtocol(Protocol):
    ...
    def route(self, shard_key: Optional[str], nodes: List[RedisNodeProtocol]) -> RedisNodeProtocol:
        """根据分片键选择节点"""
        ...
...
class MetricsCollectorProtocol(Protocol):
    ...
    def record_command(self, cmd: str, duration: float, success: bool, node_id: str) -> None:
        """记录命令执行指标"""
        ...
...
class RetryPolicy:
    ...
    retryable_exceptions: Tuple[Type[Exception], ...] = field(default_factory=lambda: (ConnectionError,))
