...
class RedisClient:
    ...
    def get_instance(
        ...
        Returns:
            RedisClient 单例实例
        """
        cache_key = f"{config.host}:{config.port}:{config.db}:{backend}"
    ...
    def _create_instance(self) -> None:
        ...
        for attempt in range(3):
            try:
                ...
                logger.warning(f"Failed to create Redis instance (attempt {attempt + 1}): {e}")
                if attempt == 2:
                    raise
    ...
    @property
    def readonly_instance(self) -> Optional[Any]:
        """获取只读实例（如果有）"""
    ...
    def _create_instance(self) -> None:
        ...
        for attempt in range(3):
            try:
                ...
                logger.warning(f"Failed to create Sentinel instance (attempt {attempt + 1}): {e}")
                if attempt == 2:
                    raise
