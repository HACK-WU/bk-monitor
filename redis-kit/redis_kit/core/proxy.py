...
from redis_kit.types import MiddlewareProtocol, RedisNodeProtocol, RetryPolicy, ShardedKey
...
class KeyExtractor:
    ...
    @classmethod
    def extract(cls, command_name: str, args: tuple, kwargs: dict) -> Optional[ShardedKey]:
        """
        从命令参数中提取键
    ...
    @classmethod
    def extract_shard_key(cls, command_name: str, args: tuple, kwargs: dict) -> Optional[str]:
        """
        从命令参数中提取分片键
...
class RedisProxy:
    ...
    def _wrap_with_middlewares(self, func: Callable, command_name: str, node_id: str) -> Callable:
        """
        使用中间件包装命令
    ...
    def execute_on_all_nodes(self, command_name: str, *args, **kwargs) -> Dict[str, Any]:
        """
        在所有节点上执行命令
        ...
        for node in self.nodes:
            ...
            try:
                ...
                logger.warning(f"Failed to execute {command_name} on {node.node_id}: {e}")
