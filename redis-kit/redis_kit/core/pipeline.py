...
class PipelineProxy:
    ...
    def __init__(self, node_proxy: "RedisProxy", transaction: bool = True):
        ...
        self._command_stack: List[str] = []  # 记录命令对应的节点 ID
    ...
    def execute(self) -> List[Any]:
        ...
        for node_id, pipeline in self._pipeline_pool.items():
            try:
                ...
                # 反转结果以便后续 pop
                node_results[node_id] = list(reversed(results))
        ...
        # 按命令顺序组装结果
        results = []
        for node_id in self._command_stack:
            node_result_list = node_results.get(node_id, [])
            if node_result_list:
                results.append(node_result_list.pop())
            else:
                results.append(None)
    ...
    def __getattr__(self, name: str) -> Callable:
        ...
        def handle(*args, **kwargs):
            ...
            # 记录命令对应的节点
            self._command_stack.append(node.node_id)
...
class TransactionProxy:
    ...
    def __init__(self, node_proxy: "RedisProxy", shard_key: Optional[str] = None):
        """
        初始化事务代理
